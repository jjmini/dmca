...
class haplotypeCaller(Workflow):
    ...
    #    bed_dir = impl.mkdir(self.option.workdir, "temp", "bed")
        ...
        #hadoop_parameter += ' -D mapreduce.map.cpu.vcores=5 '
        ...
        mapper = ["#!/bin/sh",
                  ...
                  '\t\tjava -jar %s -T HaplotypeCaller %s -I ${BAM} -L $bed -R %s -o ${VCF_TMP}/$base.g.vcf.gz' %
                  (self.haplotypeCaller.program, self.haplotypeCaller.parameter, self.ref.normal.ref),
                  "\t\tsleep 3",
                  '\t\tjava -jar %s -T GenotypeGVCFs --variant ${VCF_TMP}/$base.g.vcf.gz -R %s -o ${VCF_TMP}/$base.hc.vcf.gz' %
                  (self.haplotypeCaller.program, self.ref.normal.ref),
                  "\telse",
                  '\t\techo "Empty LANE"',
                  "\tfi",
                  "done"
        ...
        cmd = ["source %s/bin/activate" % self.GAEA_HOME,
               ...
               'allVcfs=',
               'for i in `ls ${VCF_TMP}/*g.vcf.gz`;do',
               '\tallVcfs="${allVcfs} $i"\ndone',
               '%s concat -a -O z ${allVcfs} -o ${GVCF} ' % self.haplotypeCaller.bcftools]
    #           'allVcfs=',
    #           'for i in `ls ${VCF_TMP}/*hc.vcf.gz`;do',   
    #           '\tallVcfs="${allVcfs} $i"\ndone',
    #           '%s concat -a -O z ${allVcfs} -o ${OUTPUT} ' % self.haplotypeCaller.bcftools]
