...
def single_run(args,state,rerunInfo):
    ...
    for li in rerunInfo:
        sampleName = li[0]
        jobList = getJobList(analysisDict,li[1].split(','))
            ...
            if args.type == 'local':
                ...
                for step in job.split(','):
                    ...
                    if state.hadoop.is_at_TH:
                        hold_jid = 'afterok'
                        for ds in depStep:
                            if not state.results[ds].has_key('jobId'):
#                                 printtime("No depend step JobId.")
                                continue
                            if state.results[ds].multiscript:
                                hold_jid = hold_jid + ':%s' % state.results[ds]['jobId'][state.option.multiSampleName]
                            else:
                                if state.results[step].multiscript:
                                    for sample in state.sample:
                        ...
                        jobIdDict[sampleName] = check_out_sdn(p, failFile,True)
                    else:
                        hold_jid = ''
                        for ds in depStep:
                            if not state.results[ds].has_key('jobId'):
#                                 printtime("No depend step JobId.")
                                continue
                            if state.results[ds].multiscript:
                                hold_jid += '%s,' % state.results[ds]['jobId'][state.option.multiSampleName]
                            else:
                                if state.results[step].multiscript:
                                    for sample in state.sample:
                                        hold_jid +='%s,' % state.results[ds]['jobId'][sampleName]
                                else:
                                    hold_jid += '%s,' % state.results[ds]['jobId'][sampleName]
                        ...
                        p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        jobIdDict[sampleName] = check_out_sdn(p, failFile)
                    lastStepJobId = jobIdDict[sampleName] 
                ...
                for line in p.stdout.readlines():
                    ...
                    if jobInfo:
                        ...
                        lastStepJobId = jobInfo.group(1)
                    else:
                        writefail("err happened when qsub. ", failFile)
                        exit(1) 
                p.wait()
                ...
                for step in job.split(','):
                    ...
                    for ds in depStep:
                        if not state.results[ds].has_key('jobId'):
#                             printtime("No depend step (%s) JobId." % ds)
                            continue
                        if state.results[ds].multiscript:
                            hold_jid = hold_jid + '%s,' % state.results[ds]['jobId'][state.option.multiSampleName]
                        else:
                    ...
                    if hold_jid:
                        state.results[step]['hold_jid'] = hold_jid  
                        if args.partition:
                            cmd = ['qsub','-cwd','-l',vf,'-hold_jid',hold_jid,'-q',args.queue,'-P',args.partition,'-e',sh_err,'-o',sh_out, script]
                        else:
                            cmd = ['qsub','-cwd','-l',vf,'-hold_jid',hold_jid,'-q',args.queue,'-e',sh_err,'-o',sh_out, script]
                    ...
                    lastStepJobId = jobIdDict[sampleName]
                    state.results[step]['jobId'] = jobIdDict    
            lastJobId.append(lastStepJobId)
...
#             os.environ["PATH"] = os.environ["GAEA_HOME"] + ':' + os.environ["PATH"]
        ...
        lastJobId = single_run(args,state,srerun)
        ...
        if args.type == 'submit':
            ...
            for job in multirerun:
                for step in job[1].split(','):
                    stepscript = state.results[step]['script']['multi_sample']
                    step_err = '%s.e' %stepscript
                    step_out = '%s.o' %stepscript
                    if args.partition:
                        cmd = ['qsub','-cwd','-l','vf=0.5g', '-hold_jid',lastJobId,'-q',args.queue,'-P',args.partition,'-e',step_err,'-o',step_out, stepscript]
                    else:
                        cmd = ['qsub','-cwd','-l','vf=0.5g', '-hold_jid',lastJobId,'-q',args.queue,'-e',step_err,'-o',step_out, stepscript]
                    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    for line in p.stdout.readlines():
                        printtime(line[:-1])
                        jobInfo = re.match(r'^Your job (\d+) \("(.*?)"\) has been submitted$', line)
                        if not jobInfo:
                            ...
                            hold_jid = jobInfo.group(1)
                    for line in p.stderr.readlines():
                        printtime('ERROR: (%s.sh) %s' % (step,line[:-1]))
    ...
    #                     p = subprocess.Popen("sh stepscript > step_out 2> step_err", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
